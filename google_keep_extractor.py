...
def main():
    parser = argparse.ArgumentParser(description='Export Google Keep notes to Markdown and/or PDF')
    parser.add_argument('--format', choices=['md', 'pdf', 'both'], default='md',
    ...
    if not IMPORT_PATH.exists():
        ...
        print("Please ensure you've copied the 'Takeout' folder to the same directory as this script.")
        sys.exit(1)
...
def _get_labels(note: dict[str, object]) -> list[str]:
    if 'labels' in note and isinstance(note['labels'], list):
        return [label.get('name', '') for label in note['labels'] 
...
def _note_to_pdf(note: Note, output_path: pathlib.Path) -> None:
    ...
    if note.title:
        ...
        content.append(Paragraph(note.created_at.strftime(TITLE_TIME_FORMAT), title_style))
        ...
        try:
            ...
            if img_path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.gif']:
                ...
                if not img_path.exists():
                    content.append(Paragraph(f"Missing image: {pathlib.Path(attachment).name}", styles['Italic']))
                    continue
                    
                img = Image(str(img_path), width=5*inch, height=4*inch, kind='proportional')
                content.append(img)
                content.append(Spacer(1, 0.1 * inch))
                content.append(Paragraph(f"Attachment: {pathlib.Path(attachment).name}", styles['Italic']))
            else:
                content.append(Paragraph(f"Attachment (non-image): {pathlib.Path(attachment).name}", styles['Italic']))
        except Exception as e:
            content.append(Paragraph(f"Error including attachment {attachment}: {str(e)}", styles['Italic']))
